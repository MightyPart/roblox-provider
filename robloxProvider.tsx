...
interface RobloxProviderOptions {
  ...
  redirectUri: string,
  scopes:ValidScopes[], include?:ValidIncludes[],
  checks?:ValidChecks, rest?:any
}
...
async function getAvatar(id: number) {
  return (await fetchData(`https://thumbnails.roblox.com/v1/users/avatar-headshot?userIds=${id}&size=720x720&format=Png&isCircular=false`)).data[0].imageUrl
}
...
export const RobloxProvider = (options: RobloxProviderOptions) => {
  const {clientId:CLIENT_ID, clientSecret:CLIENT_SECRET, scopes:SCOPES, include:INCLUDE, redirectUri:REDIRECT_URI, checks:CHECKS=["pkce", "state"], rest:REST} = options
